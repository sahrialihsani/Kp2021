<?php
class Model_pemerintahan extends CI_Model{
	...
	public function detail_pemerintahan($id){
		 $this->db->select('*');
    ...
    if($query->num_rows() > 0)
		...
		public function tambah_pemerintahan($data,$table){
		$this->db->insert($table, $data);
	}
	public function edit_pemerintahan($where,$table){
   ...
   $this->db->from('tb_kerjasama');
   $this->db->where('id', $id);
   $this->db->where('jenis', 'Pemerintahan');  // Also mention table name here
   $query = $this->db->get();    
   if($query->num_rows() > 0)
	   return $query->result();
   }
