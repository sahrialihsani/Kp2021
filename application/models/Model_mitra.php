...
class Model_mitra extends CI_Model{
	...
	$hasil=$this->db->query("SELECT tb_negara.id, tb_negara.name, tb_mitra.id, tb_mitra.email,tb_mitra.institusi,tb_mitra.status,tb_mitra.gambar,tb_mitra.pesan, tb_mitra.berkas FROM tb_mitra join tb_negara on tb_mitra.id_negara=tb_negara.id WHERE status='Diterima' AND tb_mitra.id_negara!=100 AND tb_mitra.gambar!='' ORDER BY RAND() LIMIT 0,24");
            return $hasil;
	}
