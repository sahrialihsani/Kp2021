...
<?= $this->session->flashdata('message'); ?>
  ...
  margin-bottom: 20px;
  font-size: 30px;
  color: #fff;">Frequently Asked Question</h2>
      ...
      <table id="example" class="table justify-image-center table-striped table-bordered" style="width:100%">
        ...
        <tbody>
              <?php
              $no = 1;
              foreach ($data_faq as $fq) { ?>
                <tr>
        <td><?=$no++ ?></td>
        <td><?=$fq->pertanyaan; ?></td>
        <td><?php echo wordwrap($fq->jawaban,100,"<br>\n");?></td>   
                  <td>
                    <div  class="btn-group">
                    <button type="button" class="btn btn-warning btn-flat btn-xs">Aksi</button>
                      ...
                      <ul style="background-color:#fff" class="dropdown-menu" role="menu">
                        <li><a style="padding-left:5px;padding-bottom:3px;padding-top:3px;color:#000" href="<?= base_url('admin/faq/editFaq/') . $fq->id; ?>"><i class="icofont-ui-edit">Edit Data</i></a></li>
                        <li><a style="padding-left:5px;padding-bottom:3px;padding-top:3px;color:#000" href="<?= base_url('admin/faq/hapusFaq/') . $fq->id; ?>" onclick="return confirm('Apakah yakin data faq ini di hapus?')"><i class="icofont-ui-delete">Hapus Data</i></a></li>
                      </ul> 
                    </div>
                  </td>
                </tr>
  ...
  <div   class="modal-dialog" role="document">
    <div class="modal-content">
      <div class="modal-header">
         <h5 style="color:#3f80ba" class="modal-title" id="exampleModalLabel">Input Data Faq</h5>
        <button type="button" class="close" data-dismiss="modal" aria-label="Close">
          <span aria-hidden="true">&times;</span>
        </button>
      </div>
      <div style="background-color:#3f80ba" class="modal-body">
      <form action="<?= base_url('admin/faq/tambahFaq');?>" method="post" enctype="multipart/form-data">
        <div class="form-group">
          <label >Pertanyaan</label>
          <input style="color:#ffffff;" type="text" name="pertanyaan" class="form-control" required>
            <?= form_error('pertanyaan','<small class="text-danger pl-3">','</small>')  ?>
         <div class="form-group">
          <label>Jawaban</label>
          <textarea style="color:#ffffff;" type="text" name="jawaban" class="form-control" required></textarea>
