...
var txt="UPT KSLI UNIB | EDIT FAQ ";
var speed=300;
var refresh=null;
...
<div class="clearfix"></div>
  ...
  <div class="content-wrapper">
    ...
    <button onclick="window.location.href='<?=base_url('admin/faq')?>'" style="border-radius:10px; margin-bottom:10px" class="btn btn-primary"><i class="icofont-arrow-left"></i> Kembali</button>
    
      <div class="box-header with-border">
        <h2 class="justify-content-center text-center" style=" font-weight: 700;
  margin-bottom: 20px;
  font-size: 30px;
  color: #fff;">Ubah data faq</h2>
        </div>
        <!-- /.box-header -->
        <?php foreach($data_faq as $fq) { ?>
          <!-- form start -->
          <form action="<?=base_url('admin/ubah_faq');?>" method="post" enctype="multipart/form-data" class="form-horizontal">
            <div class="box-body">
              <div class="form-group">
                <label class="col-sm-2 control-label">Pertanyaan</label>
                <input type="hidden" name="id" value="<?= $fq->id;?>">
                <div class="col-sm-12">
                  <input type="text" class="form-control" name="pertanyaan" value="<?= $fq->pertanyaan;?>" required>
                </div>
              </div>
              <div class="form-group">
                <label class="col-sm-2 control-label">Jawaban</label>
                <div class="col-sm-12">
                  <textarea type="text" class="form-control" name="jawaban" required><?= $fq->jawaban;?></textarea>
                </div>
              </div>
