...
class cetakpemerintahan extends CI_Controller {
	...
	function index()
	{
        ...
        $pdf->Cell(0,7,'Kerjasama dengan Pemerintahan',0,1,'C');
        $pdf->Cell(10,7,'',0,1);
        ...
        $pemerintahan = $this->db->query("SELECT tb_kerjasama.id, tb_kerjasama.nama_kerjasama,tb_mitra.institusi, tb_kerjasama.status, tb_kerjasama.file,tb_kerjasama.tgl_mulai,tb_kerjasama.tgl_akhir FROM tb_kerjasama  join tb_mitra on tb_kerjasama.id_mitra=tb_mitra.id  WHERE tb_kerjasama.jenis='Pemerintahan' ORDER BY tb_kerjasama.nama_kerjasama ASC")->result();
        $no=1;
        foreach ($pemerintahan as $data){
            $pdf->Cell(8,6,$no,1,0,'C');
